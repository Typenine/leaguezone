...
export default async function FranchiseHistoryPage({ params, searchParams }: { params: Promise<PageParams>; searchParams?: Promise<{ _league?: string }> }) {
  ...
  const scopedPath = (path: string) => getLeagueScopedPath(leagueSlug, path);
  ...
  return (
    <main className="container mx-auto max-w-[1500px] px-4 py-8">
      <div className="text-sm text-[var(--muted)]"><Link href={scopedPath('/history')} className="hover:underline">History</Link> / <Link href={scopedPath('/history/franchises')} className="hover:underline">Franchises</Link> / {franchise.teamName}</div>
      ...
      <div className="mt-3 overflow-hidden rounded-2xl border border-[var(--border)] shadow-sm">
        <div className="flex flex-col gap-5 p-5 sm:flex-row sm:items-center" style={{ background: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary || colors.primary})`, color: headerText }}>
          ...
          {franchise.currentRosterId != null ? <Link href={scopedPath(`/teams/${franchise.currentRosterId}`)} className="rounded-lg border border-white/50 bg-black/15 px-4 py-2 text-sm font-bold backdrop-blur-sm hover:bg-black/25">Current team page →</Link> : null}
        </div>
      ...
      <div className="mt-8 space-y-10">
        ...
        <Section id="milestones" title="Franchise Milestones" subtitle="Automatic career, franchise, record and championship milestones connected to this franchise.">
          <div className="grid gap-3 md:grid-cols-2">{history.milestones.length ? history.milestones.map((item) => <div key={item.id} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4"><div className="text-xs font-black uppercase tracking-wide text-[var(--muted)]">{item.season}{item.week ? ` · Week ${item.week}` : ''} · {item.type}</div><div className="mt-1 font-black">{item.title}</div><div className="mt-1 text-sm text-[var(--muted)]">{item.detail}</div>{item.week ? <Link href={scopedPath(`/history/gamebook/${item.season}/${item.week}`)} className="mt-2 inline-block text-xs font-bold text-[var(--accent)] hover:underline">Open that week →</Link> : null}</div>) : <div className="rounded-lg border border-dashed border-[var(--border)] p-6 text-sm text-[var(--muted)]">No qualifying milestones yet.</div>}</div>
          <Link href={scopedPath('/history/milestones')} className="inline-block text-sm font-bold text-[var(--accent)] hover:underline">View league-wide milestones →</Link>
        </Section>
      </div>
