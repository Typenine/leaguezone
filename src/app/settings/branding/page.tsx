...
import HistoricalBrandEditor from '@/components/branding/HistoricalBrandEditor';
...
export default function BrandingSettingsPage() {
  ...
  const saveLeague = async () => {
    ...
    try {
      const res = await fetch('/api/settings/branding', {
        ...
        body: JSON.stringify({ logoUrl: league.logoUrl || '', primaryColor: league.primaryColor || '', secondaryColor: league.secondaryColor || '' }),
      });
      const body = await res.json().catch(() => ({}));
  ...
  const saveTeam = async () => {
    ...
    try {
      const res = await fetch('/api/settings/team', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ logoUrl: team.logoUrl || '', primaryColor: team.primaryColor || '', secondaryColor: team.secondaryColor || '', tertiaryColor: team.tertiaryColor || '', quaternaryColor: team.quaternaryColor || '' }),
      });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error || 'Save failed');
  ...
  if (!league) return <div className="container mx-auto max-w-5xl px-4 py-10 text-[var(--muted)]">Loading branding settings…</div>;
  ...
  return (
    <div className="container mx-auto max-w-5xl space-y-6 px-4 py-8 sm:py-10">
          ...
          <CardContent className="space-y-5">
            <div className="grid gap-5 md:grid-cols-[180px_1fr]">
              <div>
                ...
                <label className="mt-3 inline-flex cursor-pointer rounded-lg border border-[var(--border)] px-3 py-2 text-xs font-bold text-[var(--text)] hover:border-[var(--accent)]">Upload image<input className="hidden" type="file" accept="image/png,image/jpeg,image/webp" onChange={async (event) => {
                  const file = event.target.files?.[0]; if (!file) return;
                  try { setBusy(true); const url = await uploadBrandImage(file, 'league-logo'); setLeague((prev) => prev ? { ...prev, logoUrl: url } : prev); setLeagueStatus('Image uploaded. Save branding to apply it.'); }
                  catch (error) { setLeagueStatus(error instanceof Error ? error.message : 'Upload failed'); }
          ...
          <CardContent className="space-y-5">
            <div className="grid gap-5 md:grid-cols-[180px_1fr]">
              <div>
                ...
                <div className="mt-2 flex h-36 w-36 items-center justify-center overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)]">{team.logoUrl ? <img src={team.logoUrl} alt="Team logo preview" className="h-full w-full object-contain" /> : <span className="text-xs text-[var(--muted)]">Default helmet</span>}</div>
                <label className="mt-3 inline-flex cursor-pointer rounded-lg border border-[var(--border)] px-3 py-2 text-xs font-bold text-[var(--text)] hover:border-[var(--accent)]">Upload image<input className="hidden" type="file" accept="image/png,image/jpeg,image/webp" onChange={async (event) => {
                  const file = event.target.files?.[0]; if (!file) return;
                  try { setBusy(true); const url = await uploadBrandImage(file, 'team-logo'); setTeam((prev) => prev ? { ...prev, logoUrl: url } : prev); setTeamStatus('Image uploaded. Save team branding to apply it.'); }
                  catch (error) { setTeamStatus(error instanceof Error ? error.message : 'Upload failed'); }
                  finally { setBusy(false); event.target.value = ''; }
                }} /></label>
              ...
              <div className="grid content-start gap-4 sm:grid-cols-2">
                {([['primaryColor', 'Primary', '#1a1a2e'], ['secondaryColor', 'Secondary', '#16213e'], ['tertiaryColor', 'Tertiary (optional)', '#333333'], ['quaternaryColor', 'Quaternary (optional)', '#444444']] as const).map(([key, label, fallback]) => (
                  <div key={key}><Label>{label}</Label><div className="mt-1 flex gap-2"><input type="color" value={team[key] || fallback} onChange={(e) => setTeam({ ...team, [key]: e.target.value })} className="h-10 w-12 rounded border border-[var(--border)]" /><Input value={team[key] || ''} onChange={(e) => setTeam({ ...team, [key]: e.target.value })} placeholder={fallback} /></div></div>
                ))}
              </div>
            </div>
      ...
      {league.canManage ? <Card><CardHeader><CardTitle>Historical franchise branding</CardTitle></CardHeader><CardContent><HistoricalBrandEditor /></CardContent></Card> : null}
      ...
      {league.slug ? (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)]">{cardUrl ? <img src={cardUrl} alt="League-branded share card preview" className="block w-full" /> : null}</div>
            <div className="flex flex-wrap gap-3">
              <a href={cardUrl} target="_blank" rel="noreferrer" className="inline-flex rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-[var(--on-accent,#fff)]">Open full-size card</a>
