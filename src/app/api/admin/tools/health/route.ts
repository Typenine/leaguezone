...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const tableChecks = await db.execute(sql`
      ...
      ORDER BY table_name
    `);
    const tables = ((tableChecks as { rows?: Array<Record<string, unknown>> }).rows ?? []).map(
    ...
    await Promise.all(
      coreCountQueries.map(async ([t, q]) => {
  ...
  try {
    const db = getDb();
    const setupRes = await db.execute(sql`
      SELECT id, setup_completed, name, config FROM leagues ORDER BY created_at DESC LIMIT 1
    `);
  ...
  const r2Configured = Object.values(r2Vars).every(Boolean);
  results.r2 = { configured: r2Configured, vars: r2Vars };
  ...
  // --- Resend (email) ---
  results.resend = { configured: !!process.env.RESEND_API_KEY?.trim() };
