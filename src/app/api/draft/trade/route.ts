...
export async function GET(req: NextRequest) {
  try {
    ...
    if (action === 'get_assets') {
      const adminReq = await isDraftAdmin(req);
      const ident = adminReq ? null : await requireTeamUser().catch(() => null);
      if (!adminReq && !ident) return bad('auth_required', 401);
    ...
    if (action === 'get_admin_pending') {
      if (!(await isDraftAdmin(req))) return bad('forbidden', 403);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'clear_trade_animation') {
      if (!adminReq && !myTeam) return bad('auth_required', 401);
