...
export default function AdminDraftsPage() {
  ...
  async function action(body: Record<string, unknown>) {
    ...
    setSaving(false);
    if (!res.ok) { setMessage(result.error || 'Draft action failed'); return null; }
    await load();
    return result;
  }
  async function create() {
    const result = await action({ action: 'create', leagueId, year: Number(year), rounds: Number(rounds), clockSeconds: Number(clockSeconds), playerPoolType });
    if (result) setMessage(result.warning || `${year} draft created. It is independent of Sleeper league creation and ready for setup.`);
  }
  async function openDraft(draft: Draft, destination: string) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mb-6 grid gap-4 lg:grid-cols-[.8fr_1.2fr]">
        <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 className="font-black text-[var(--text)]">League</h2><select value={leagueId} onChange={(e) => setLeagueId(e.target.value)} className="mt-3 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-3 py-2.5 text-sm text-[var(--text)]">{data.leagues.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{league && <div className="mt-3 flex flex-wrap gap-2 text-xs"><Link href={`/admin/qa?league=${encodeURIComponent(league.id)}`} className="rounded-lg border border-amber-500/30 px-2.5 py-1.5 text-amber-500">QA This League</Link><Link href={`/l/${league.slug}/draft`} className="rounded-lg border border-[var(--border)] px-2.5 py-1.5">League Draft Page</Link></div>}</section>
        <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><h2 className="font-black text-[var(--text)]">Create Future / Live Draft</h2><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><label className="text-xs font-semibold text-[var(--muted)]">Year<input type="number" value={year} onChange={(e) => setYear(e.target.value)} className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 py-2 text-sm text-[var(--text)]"/></label><label className="text-xs font-semibold text-[var(--muted)]">Rounds<input type="number" min="1" max="40" value={rounds} onChange={(e) => setRounds(e.target.value)} className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 py-2 text-sm text-[var(--text)]"/></label><label className="text-xs font-semibold text-[var(--muted)]">Clock (sec)<input type="number" min="10" value={clockSeconds} onChange={(e) => setClockSeconds(e.target.value)} className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 py-2 text-sm text-[var(--text)]"/></label><label className="text-xs font-semibold text-[var(--muted)]">Draftable players<select value={playerPoolType} onChange={(e) => setPlayerPoolType(normalizeDraftPlayerPoolType(e.target.value))} className="mt-1 w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-2 py-2 text-sm text-[var(--text)]">{DRAFT_PLAYER_POOL_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div><p className="mt-2 text-xs text-[var(--muted)]">{DRAFT_PLAYER_POOL_OPTIONS.find((option) => option.value === playerPoolType)?.description}</p><button disabled={saving || !leagueId} onClick={create} className="mt-4 rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-black text-white disabled:opacity-50">Create Draft</button></section>
      </div>

      <section className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-black text-[var(--text)]">{league?.name || 'League'} Drafts</h2><span className="text-xs text-[var(--muted)]">{rehearsalCount} rehearsal session{rehearsalCount === 1 ? '' : 's'} stored separately</span></div>{loading ? <p className="mt-4 text-sm text-[var(--muted)]">Loading…</p> : <div className="mt-4 space-y-3">{liveDrafts.length === 0 ? <p className="text-sm text-[var(--muted)]">No LeagueZone-managed drafts yet.</p> : liveDrafts.map((draft) => <div key={draft.id} className={`rounded-xl border p-4 ${draft.archivedAt ? 'border-[var(--border)] opacity-70' : 'border-[var(--border)]'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className="text-lg font-black text-[var(--text)]">{draft.year} Draft</span><span className="rounded-full bg-[var(--background)] px-2 py-0.5 text-[10px] font-black uppercase text-[var(--muted)]">{draft.archivedAt ? 'Archived' : draft.status}</span></div><p className="text-xs text-[var(--muted)]">{draft.rounds} rounds · {draft.clockSeconds}s clock · created {new Date(draft.createdAt).toLocaleDateString()}</p></div>{!draft.archivedAt && <div className="flex flex-wrap gap-2 text-xs font-semibold"><button onClick={() => openDraft(draft, '/draft/room')} className="rounded-lg bg-[var(--accent)] px-3 py-2 text-white">Open Draft Room</button>{league && <button onClick={() => openDraft(draft, `/l/${encodeURIComponent(league.slug)}/admin/draft`)} className="rounded-lg border border-[var(--border)] px-3 py-2">Commissioner Console</button>}<Link href={`/admin/qa?league=${encodeURIComponent(leagueId)}&year=${draft.year}&sourceDraft=${encodeURIComponent(draft.id)}`} className="rounded-lg border border-amber-500/30 px-3 py-2 text-amber-500">Rehearse</Link>{draft.status === 'COMPLETED' && <button onClick={() => archive(draft)} className="rounded-lg border border-[var(--border)] px-3 py-2">Archive</button>}</div>}</div></div>)}</div>}</section>
