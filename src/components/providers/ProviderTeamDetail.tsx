...
export default async function ProviderTeamDetail({ leagueId, leagueSlug, rosterId, season }: { leagueId: string; leagueSlug: string; rosterId: number; season?: string | null }) {
  ...
  return (
    <div className="container mx-auto space-y-6 px-4 py-8">
      <SectionHeader title={detail.team.teamName} subtitle={`${detail.season} · ${detail.provider === 'yahoo' ? 'Yahoo Fantasy' : 'Sleeper'} · ${record}`} />
      ...
      <Card>
        ...
        <CardContent>
          {detail.players.length === 0 ? <p className="text-sm text-[var(--muted)]">No roster data is available.</p> : (
            <div className="divide-y divide-[var(--border)]">
              {detail.players.sort((a, b) => (a.position || '').localeCompare(b.position || '') || a.fullName.localeCompare(b.fullName)).map((player) => (
                <Link key={player.playerId} href={`/l/${leagueSlug}/players/${encodeURIComponent(player.playerId)}`} className="flex items-center justify-between gap-4 py-3 hover:text-[var(--accent)]">
                  <span className="font-semibold">{player.fullName}</span><span className="text-sm text-[var(--muted)]">{[player.position, player.nflTeam].filter(Boolean).join(' · ') || 'Player'}</span>
                </Link>
              ))}
            </div>
          )}
